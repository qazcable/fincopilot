  ...
  for (let start = 0; start < names.length; start += batchSize) {
    ...
    try {
      const response = await withRetry(() => ai.models.generateContent({
        ...
        config: {
          systemInstruction: `Это описания трат пользователя из Казахстана: названия получателей платежей по карте (магазины, кафе, ИП, сервисы) или короткие записи вроде «сигареты», «стрижка», «подарок маме».
Для каждого номера выбери самую подходящую категорию из списка. ИП и ТОО с непонятным названием — «Другое».
Категории:
${categoryList}`,
          thinkingConfig: THINKING,
          responseMimeType: "application/json",
