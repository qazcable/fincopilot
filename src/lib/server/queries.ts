...
export async function getHistory(user: AppUser, year: number, month: number) {
  ...
  return {
    items,
    expense: items.filter(t => t.kind === "EXPENSE").reduce((s, t) => s + t.amount, 0),
    income: items.filter(t => t.kind === "INCOME").reduce((s, t) => s + t.amount, 0),
  };
}
...
export async function getStats(user: AppUser, year: number, month: number) {
  ...
  const [transactions, previousExpense] = await Promise.all([
    ...
    prisma.transaction.aggregate({
      where: { userId: user.id, kind: "EXPENSE", occurredAt: { gte: previous.from, lt: previous.to } },
      _sum: { amount: true },
    }),
  ]);
